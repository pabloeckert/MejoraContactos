...
`motor-contactos/` is a separate, private Python project (AI-assisted contact dedup/normalization pipeline for the repo owner's personal contacts) living alongside the SPA in this repo, gitignored/untracked on purpose.

**If resuming work on motor-contactos in a new session (new account, ran out of credit, etc.): STOP and read `motor-contactos/PROMPT_CONTINUACION.md` first, follow it literally.** That file is the maintained, current entry point — it points to `ESPECIFICACION.md` (architecture), `PENDIENTES.md` (live task list, always current), the latest file in `handoffs/` (auto-generated status report), and the last entries of `DECISIONES.md` (append-only decision log). Do not rely on this CLAUDE.md section for details beyond this pointer — it is intentionally NOT kept in sync session-to-session; the files above are, and `motor-contactos/` has its own local-only git repo (no remote) that `scripts/handoff.ps1` commits to at each handoff.

`MejoraContactos.md` at the repo root (gitignored) still holds full narrative transcripts of past sessions if deeper historical context is ever needed, but it is not the continuation mechanism — `PROMPT_CONTINUACION.md` is.
