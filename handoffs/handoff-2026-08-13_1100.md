...
## Commits desde el handoff anterior (handoff-2026-08-13_1053)

```
0f8010f wip: snapshot antes de handoff (2026-08-13_1100)
```

### Archivos modificados

```
 PENDIENTES.md                       |  35 +++--  handoffs/handoff-2026-08-13_1053.md | 301 ++++++++++++++++++++++++++++++++++++  2 files changed, 322 insertions(+), 14 deletions(-)
```
...
## Bloqueante único para cerrar el MVP (verificado 2026-08-13, corte de control)

- [ ] **Revisar los 649 pendientes — es UNA sola decisión, no 649.** Los
      649 casos son el MISMO patrón exacto (`tel=si|mail=no|nombre=baja|
      nombres_distintos`, 100% de ellos): mismo teléfono, sin email para
      desempatar, nombres que el sistema lee como distintos. En `/revisar`
      aparecen agrupados en un solo lote — un click en "Aprobar fusión de
      todo el lote" o "Rechazar fusión de todo el lote" cierra los 649 de
      una vez. Criterio para decidir: si en tu base es común que dos
      personas distintas compartan un teléfono (línea familiar/de
      oficina), rechazar el lote es lo seguro; si tu experiencia es que un
      mismo teléfono casi siempre es la misma persona con el nombre
      escrito distinto, aprobarlo es más rápido y el aprendizaje
      (`dedup/learning.py`) ajusta el umbral con esa señal para el futuro.
      Esta es la única decisión que falta para tener una lista maestra
      100% resuelta (0 pendientes). Una vez decidido, NO volver a correr
      `deduplicar` (pisa la decisión humana — ver ESPECIFICACION.md §
      dedup).
